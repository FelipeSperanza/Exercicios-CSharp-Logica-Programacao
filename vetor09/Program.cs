...
namespace vetor09
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            for (int i = 0; i < n; i++)
            {
                if ((precoVenda[i] - precoCompra[i]) / precoCompra[i] * 100 < 10)
                {
                    abaixo += 1;
                }
                else if((precoVenda[i] - precoCompra[i]) / precoCompra[i] * 100 >= 10 && (precoVenda[i] - precoCompra[i]) / precoCompra[i] * 100 <= 20)
                {
                    entre += 1;
                }
                else
